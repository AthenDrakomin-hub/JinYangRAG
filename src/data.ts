...
该匹配算法完全运行于 CPU 极其底层的数据结构，耗时通常只需零点几微秒，不需要加载庞大且昂贵的矩阵运算库即可对网页短文本完成极致速度的本地搜寻。`
  ...
  {
    id: "agnes-flash",
    title: "Agnes-2.0-Flash 创新大模型特性深度介绍",
    url: "https://api.agnes-ai.com/v1",
    category: "前沿模型",
    content: `Agnes-2.0-Flash 是专为大规模轻盈化设计 and 极速自然语言理解精雕细琢的高性能轻量大尺寸模型。
Agnes-2.0-Flash 拥有一流的多模态理解眼界，能够在单个会话中流畅解读海量文本。
它提供了巨大的百万级 Token 默认上下文容量，使得分析数十万字的长文档或者极其庞大的语料库检索变得唾手可得。
此外，Agnes-2.0-Flash 在时效控制和硬件开销取得了极度微妙的平衡，其在常态长问答和结构化 RAG 复杂知识检索过滤上响应迅捷。
配合高效的系统指令功能，可以非常精确地让模型承担销冠专家角色，是开发者进行在线 RAG 开发、智慧服务设计的最理想、性价比最高的大语言模型。`
  },
  {
    id: "wechat-web-im",
...
本系统将自主通过前端 MutationObserver 对此 IM 区块执行实时消息捕获，并联动 Supabase pgvector 本地知识库和 Agnes AI，在侧边栏为您实时呈现“金牌销售推荐话术推荐”，大幅提升客户成交概率。`
  }
];
