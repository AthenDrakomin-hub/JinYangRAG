...
4. 推理阶段：把拼接好的高质量提示词喂给像 Gemini 3.5 如此优秀强大的大模型，此时模型能获取极度可靠的实时参考，仅依赖已有事实提供准确无误的回复。`
    };
...
function appendMessageBubble(sender, text, sources = []) {
  ...
  senderLabel.textContent = sender === "user" ? "您提问" : "Gemini 3.5 AI 助手";
...
function appendLoadingBubble() {
  ...
  const senderLabel = document.createElement("span");
  senderLabel.className = "message-sender";
  senderLabel.textContent = "Gemini正在计算回复...";
  bubble.appendChild(senderLabel);
